...
public class YtgldFeatureRenderer <T extends ytgld, M extends YtgldEntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    public YtgldFeatureRenderer(FeatureRendererContext<T, M> context, Identifier texture, AnimationAngleAdjuster<T> animationAngleAdjuster, ModelPartVisibility<T, M> modelPartVisibility) {
        super(context);
    ...
    public void updateModelPartVisibility() {
    ...
    public void unhideAllModelParts() {
        (this.getContextModel()).getPart().traverse().forEach((part) -> {
            part.hidden = false;
        });
    }
    ...
    public interface AnimationAngleAdjuster<T extends nightmare_giant> {
        float apply(T warden, float tickDelta, float animationProgress);
    }

    @Environment(EnvType.CLIENT)
    public interface ModelPartVisibility<T extends nightmare_giant, M extends EntityModel<T>> {
